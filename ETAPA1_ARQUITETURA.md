...
8. ✅ **Relatórios ricos reutilizáveis** (`src/reports/`, Seção 28) — 3 de 3 tipos concluídos.
   ...
   - ✅ **Extensão: 2018 passou a ter comparativo territorial real** (`src/candidate_history.py`). O TSE bloqueia (403) o download oficial de `votacao_secao_2018`; baixado o fallback já documentado em `config/data_sources.yaml` (mirror `data.brasil.io`, `votacao_candidato_munzona`, nível candidato×município×zona×turno, só voto nominal), convertido para parquet nacional reduzido (`scripts/converter_munzona_2018.py`, 8.986.056 linhas, publicado na release `dados-v2` e no `pacote_cloud.arquivos`), e plugado como uma segunda fonte de carregamento (`votos_da_disputa_munzona_2018`/`votos_da_candidatura_munzona_2018`) roteada dentro de `_carregar_disputa()` quando o ano comparável é 2018. **Bug real corrigido no processo**: o gate que decidia "tem território ou não" usava `Candidatura.zonas_com_votos` — um campo preenchido por uma consulta a `votacao_secao` que *sempre* falha pra 2018 (retornando 0 mesmo quando o munzona tem dado real) — trocado por uma checagem no DataFrame de votos efetivamente carregado. Consequência do mesmo bug: o total de votos exibido no relatório usava `Candidatura.total_votos` (também sempre 0 para 2018) — corrigido com um campo novo (`votos_totais_comparavel`, somado do território real já calculado) usado no lugar disso no renderizador. Nova limitação declarada sempre que 2018 entra numa comparação (voto nominal, sem seção, fonte alternativa). Validado com caso real: SANDERSON RIBEIRO CORREIA DE LIMA, Deputado Estadual/SP 2018 (suplente, AVANTE) → Deputado Federal/SP 2022 (PRTB, não eleito) — 29 zonas em comum, correlação territorial real 0,956. 1 teste novo, 253 no total, zero regressão.
